...
cd "$submit_dir"
echo $submit_dir
echo "Submitting synthetic data simulations"

total_number=0

#Submit a job for each line
condor_submit <(
    echo "Description =\"HCWFSYNTH\""
    ...
    echo "Getenv                           = True"
    echo "Executable                       = /mnt/condor/rverschoren/SimHCWF.exe"
    ...
    do
        ...
        echo "Arguments             = \"$tl\""
        echo "Log                   = $dashtl.hcwf.log"
        echo "Output                = $dashtl.hcwf.out"
        echo "Queue"
    done < paramshcwf.csv
) >/dev/null

condor_submit <(
    echo "Description =\"COLDSYNTH\""
    echo "Requirements                     = TARGET.Arch             == \"X86_64\" \\"
    echo "                                && TARGET.FileSystemDomain == \"controller.mosaic.uantwerpen.be\""
    echo "Rank                             = TARGET.KFlops"
    echo "Request_Memory                   = 2048 Mb"
    ...
    echo "Universe                         = Vanilla"
    echo "Getenv                           = True"
    echo "Executable                       = /mnt/condor/rverschoren/SimCOLD.exe"
    echo "Should_Transfer_Files            = No"
...
condor_submit <(
    ...
    echo "Universe                         = Vanilla"
    echo "Getenv                           = True"
    echo "Executable                       = /mnt/condor/rverschoren/SimDWF.exe"
    echo "Should_Transfer_Files            = No"
